<template>
  ...
  <div>
    ...
    >
      ...
      <el-table-column label="商品详情" width="220">
        <template slot-scope="props">
          <span class="details" style="font-size: 14px;"> {{ props.row.item_detail }} </span>
        </template>
      </el-table-column>

      <el-table-column label="成色分类" width="150">
        <template slot-scope="props">
          <span class="kind"> {{ props.row.fineness }}
              <br />
              {{ props.row.main_kind }}-{{ props.row.sub_kind }} </span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="金额" width="180" prop="total">
        <template slot-scope="props">
      ...
      <el-table-column label="操作" width="200">
        <template slot-scope="scope">
          <el-button
            @click="goodpage(scope.row.item_name)"
            type="text"
            size="small"
            >查看商品详情</el-button
          >
    ...
    <el-footer>
      <el-col :span="4" :offset="16" class="footer_left"
        >合计：
        <span>¥ {{ num_price }}</span>
      </el-col>
...
export default {
  ...
  methods: {
    ...
    async deleteItem(shopcar) {
      ...
      const { data: res } = await this.$http.post("deleteShopCar", shopcar);
      if (res == "success") {
